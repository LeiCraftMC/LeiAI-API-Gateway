...
describe("OpenAI-Compatible Fake Backend Integration", () => {
  ...
  describe("Basic chat completion", () => {
    it("should forward a non-streaming chat completion", async () => {
  ...
  describe("Streaming chat completion", () => {
    it("should forward SSE chunks", async () => {
      provider = new Provider(
        {
          name: "my-provider",
          backends: [fake.toBackendConfig("fake-backend")],
  ...
  describe("Provider prefix stripping", () => {
    it("should strip the provider prefix before forwarding", async () => {
      provider = new Provider(
        {
          name: "custom-prefix",
  ...
  describe("Load balancing", () => {
    ...
    it("should distribute requests across two backends", async () => {
      provider = new Provider(
        {
          name: "my-provider",
          backends: [
  ...
  describe("Health checks", () => {
    it("should mark a backend unhealthy after repeated failures", async () => {
      provider = new Provider(
        {
          name: "my-provider",
    ...
    it("should recover a backend after it starts succeeding", async () => {
      provider = new Provider(
        {
          name: "my-provider",
  ...
  describe("Error masking", () => {
    it("should mask a backend 500 as 500 Internal Server Error", async () => {
      provider = new Provider(
        {
          name: "my-provider",
  ...
  describe("Timeout masking", () => {
    it("should mask a backend timeout as 500", async () => {
      closeFakeBackend(fake);
  ...
  describe("Other OpenAI endpoints", () => {
    it("should forward /v1/completions", async () => {
      provider = new Provider(
        {
          name: "my-provider",
          backends: [fake.toBackendConfig("fake-backend")],
    ...
    it("should forward /v1/embeddings", async () => {
      provider = new Provider(
        {
          name: "my-provider",
          backends: [fake.toBackendConfig("fake-backend")],
...
describe("OpenAI-Compatible Full Server Integration", () => {
  ...
  it("should route through the full server stack", async () => {
  ...
  it("should round-robin across backends via the full server", async () => {
    const base = `http://${server.hostname}:${server.port}`;
  ...
  it("should stream through the full server stack", async () => {
    const base = `http://${server.hostname}:${server.port}`;
    const response = await fetch(`${base}/my-provider/v1/chat/completions`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
